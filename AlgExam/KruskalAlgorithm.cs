...
namespace AlgExam
{
    public class KruskalAlgorithm
    {
        ...
        public void KruskalMST()
        {
            ...
            int Find(int i)
            {
                // Если узел i является корнем своего дерева, возвращаем его
                if (parent[i] == i)
                    return i;
                else
                {
                    // Иначе, находим корень родительского узла и обновляем ссылку,
                    // чтобы узел i указывал непосредственно на корень,
                    // это оптимизирует производительность алгоритма
